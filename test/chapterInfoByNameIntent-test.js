...
test('ChapterInfoByNameIntent for Surah Al-Faatiha', t => {
  const event = Request.intent('ChapterInfoByNameIntent', { MEEZAN_CHAPTERNAME: 'Al-Faatiha' }).build();
...
test('ChapterInfoByNameIntent for Surah Yusuf ', t => {
  const event = Request.intent('ChapterInfoByNameIntent', { MEEZAN_CHAPTERNAME: 'Yusuf' }).build();
